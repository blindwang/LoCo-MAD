...
@registry.register_task("gen_vqa")
class GEN_VQATask(BaseTask):
    def __init__(
        self,
        num_beams,
        max_len,
        ...
        self.coco_fmt_file: dict = coco_fmt_file
    ...
    def setup_task(cls, cfg):
        ...
        num_ans_candidates = run_cfg.get("num_ans_candidates", 128)
        prompt = run_cfg.get("prompt", "")
        coco_fmt_file = run_cfg.get("coco_fmt_file", None)
    ...
    def build_datasets(self, cfg):
        ...
        for dataset in datasets.values():
            for split in dataset:
                ...
                if self.coco_fmt_file is not None and split in self.coco_fmt_file:
                    ques_f = Path(self.coco_fmt_file[split]['questions'])
                    if not ques_f.is_absolute():
                        ques_f = Path(registry.get_path('cache_root')) / ques_f
                    anno_f = Path(self.coco_fmt_file[split]['annotations'])
                    if not anno_f.is_absolute():
                        anno_f = Path(registry.get_path('cache_root')) / anno_f
                    self.ques_files[split] = str(ques_f)
                    self.anno_files[split] = str(anno_f)
...
class VideoQATask(BaseTask):
    ...
    def valid_step(self, model, samples):
        ...
        question_id = samples["question_id"]
        for answer, ques_id, gt_answer, ques in zip(answers, question_id, samples['answers'], samples['text_input']):
            # post-processing
            # if answer.startswith('a '):
            #     answer = answer[2:]
            pred_qa_pairs.append({"question_id": ques_id, "question": ques,
    ...
    def _report_metrics(self, result_file, split):
        ...
        vqa_tool = VQAEval()
        total_cnt, pos_cnt = 0, 0

        for res in results:
            total_cnt += 1
            answer = res['answer']
            answer = vqa_tool.processPunctuation(answer)
            answer = vqa_tool.processDigitArticle(answer)
            if answer == res['gt_answer']:
                pos_cnt += 1

        acc = pos_cnt / total_cnt
        metrics['agg_metrics'] = acc
        metrics['accuracy'] = acc
        logging.info(f"Overall Accuracy is: {acc:.2f}")
